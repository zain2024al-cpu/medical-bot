...
CHOOSE_DELETE_METHOD, GET_DELETE_PARAM, CONFIRM_DELETE = range(9, 12)
...
def create_summary_table(hospitals_data):
    ...
    sorted_hospitals = sorted(hospitals_data.items(), key=lambda x: x[1], reverse=True)
...
async def generate_comparison_pdf(update: Update, hospitals_data, departments_data, hospital_filter, month_filter):
    setup_pdf_fonts()

    buffer = BytesIO()
    doc = SimpleDocTemplate(buffer, pagesize=landscape(A4), rightMargin=20, leftMargin=20, topMargin=50, bottomMargin=20)
    ...
    elements.append(Paragraph(arabic_text("جدول الإجمالي (عدد العمليات لكل مستشفى)"), subtitle_style))

    sorted_hospitals = sorted(hospitals_data.items(), key=lambda x: x[1], reverse=True)
    summary_data = [
        [arabic_text("المستشفى"), arabic_text("عدد العمليات")]
    ]
    ...
    elements.append(Paragraph(arabic_text("الجدول المفصل (الأقسام × المستشفيات)"), subtitle_style))
    ...
    elements.append(Spacer(1, 20))
    elements.append(Paragraph(arabic_text("الرسم البياني للمقارنة"), subtitle_style))
...
async def generate_hospital_comparison(update: Update, context: ContextTypes.DEFAULT_TYPE):
    args = context.args
    ...
    if not data:
        await update.message.reply_text(rtl("⚠️ لا توجد بيانات للمقارنة بناءً على الفلاتر المحددة"))
        return
    ...
    pdf_buffer = await generate_comparison_pdf(update, hospitals_data, departments_data, hospital_filter, month_filter)
...
async def report_rtl_final(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for row in data:
        ...
        if month and row_month != month:
            continue
        if hospital_filter and hospital_filter.lower() not in row_hospital.lower():
            continue
        filtered.append(row)
    ...
    buffer = io.BytesIO()
    doc = SimpleDocTemplate(buffer, pagesize=A4, rightMargin=20, leftMargin=20, topMargin=80, bottomMargin=20)
    ...
    elements.append(Paragraph(arabic_text("تقرير العمليات الطبية"), style_title))
    ...
    headers = ["م", "التاريخ", "المريض", "المستشفى", "القسم", "الطبيب", "الإجراء", "المبلغ", "التخفيض", "العمولة"]
    headers_rtl = list(reversed(headers))
    table_data = [[Paragraph(arabic_text(h), style_table) for h in headers_rtl]]
    ...
    doc.build(elements)
    buffer.seek(0)
    await update.message.reply_document(document=InputFile(buffer, filename="report_rtl_final.pdf"))
...
# ========= دوال الحذف الجديدة =========
async def delete_operations(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """بدء عملية الحذف مع تحديد المعايير"""
    keyboard = [
        ['حذف الكل', 'حذف حسب الشهر'],
        ['حذف حسب التاريخ', 'حذف حسب المستشفى'],
        ['إلغاء الحذف']
    ]
    reply_markup = ReplyKeyboardMarkup(keyboard, resize_keyboard=True)

    await update.message.reply_text(
        rtl("🔴 اختر طريقة الحذف:"),
        reply_markup=reply_markup
    )
    return CHOOSE_DELETE_METHOD

async def choose_delete_method(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """معالجة اختيار طريقة الحذف"""
    choice = update.message.text

    if choice == "حذف الكل":
        await update.message.reply_text(
            rtl("⚠️ هل أنت متأكد من حذف جميع العمليات؟ اكتب 'نعم' للتأكيد أو 'لا' للإلغاء:"),
            reply_markup=ReplyKeyboardRemove()
        )
        context.user_data["delete_method"] = "all"
        return CONFIRM_DELETE

    elif choice == "حذف حسب الشهر":
        await update.message.reply_text(
            rtl("📅 أدخل رقم الشهر (1-12) للحذف:"),
            reply_markup=ReplyKeyboardRemove()
        )
        context.user_data["delete_method"] = "month"
        return GET_DELETE_PARAM

    elif choice == "حذف حسب التاريخ":
        await update.message.reply_text(
            rtl("📅 أدخل التاريخ (YYYY-MM-DD) للحذف:"),
            reply_markup=ReplyKeyboardRemove()
        )
        context.user_data["delete_method"] = "date"
        return GET_DELETE_PARAM

    elif choice == "حذف حسب المستشفى":
        await update.message.reply_text(
            rtl("🏥 أدخل اسم المستشفى للحذف:"),
            reply_markup=ReplyKeyboardRemove()
        )
        context.user_data["delete_method"] = "hospital"
        return GET_DELETE_PARAM

    elif choice == "إلغاء الحذف":
        await update.message.reply_text(
            rtl("✅ تم إلغاء عملية الحذف"),
            reply_markup=ReplyKeyboardRemove()
        )
        return ConversationHandler.END

async def get_delete_param(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """الحصول على معيار الحذف"""
    param = update.message.text
    context.user_data["delete_param"] = param

    # التحقق من صحة المدخلات
    method = context.user_data["delete_method"]

    if method == "month":
        try:
            month = int(param)
            if month < 1 or month > 12:
                await update.message.reply_text(rtl("⚠️ رقم الشهر يجب أن يكون بين 1 و 12:"))
                return GET_DELETE_PARAM
        except ValueError:
            await update.message.reply_text(rtl("⚠️ أدخل رقم صحيح للشهر:"))
            return GET_DELETE_PARAM

    elif method == "date":
        if not re.match(r'^\d{4}-\d{2}-\d{2}$', param):
            await update.message.reply_text(rtl("⚠️ أدخل التاريخ بصيغة YYYY-MM-DD:"))
            return GET_DELETE_PARAM

    # عرض عدد العمليات التي سيتم حذفها
    count = await get_operations_count(context.user_data["delete_method"], param)

    if count == 0:
        await update.message.reply_text(rtl("⚠️ لا توجد عمليات تطابق معيار الحذف"))
        return ConversationHandler.END

    await update.message.reply_text(
        rtl(f"⚠️ سيتم حذف {count} عملية. اكتب 'نعم' للتأكيد أو 'لا' للإلغاء:")
    )
    return CONFIRM_DELETE

async def get_operations_count(method, param):
    """الحصول على عدد العمليات التي تطابق معيار الحذف"""
    if method == "all":
        cursor.execute("SELECT COUNT(*) FROM operations")
        return cursor.fetchone()[0]

    elif method == "month":
        cursor.execute("SELECT COUNT(*) FROM operations WHERE strftime('%m', date) = ?",
                      (f"{int(param):02d}",))
        return cursor.fetchone()[0]

    elif method == "date":
        cursor.execute("SELECT COUNT(*) FROM operations WHERE date = ?", (param,))
        return cursor.fetchone()[0]

    elif method == "hospital":
        cursor.execute("SELECT COUNT(*) FROM operations WHERE hospital LIKE ?",
                      (f"%{param}%",))
        return cursor.fetchone()[0]
...
async def confirm_delete(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if confirmation == 'نعم':
        method = context.user_data["delete_method"]
        param = context.user_data.get("delete_param", "")

        deleted_count = await execute_delete(method, param)

        await update.message.reply_text(
            rtl(f"✅ تم حذف {deleted_count} عملية بنجاح")
        )

    elif confirmation == 'لا':
        await update.message.reply_text(rtl("✅ تم إلغاء عملية الحذف"))
    else:
        await update.message.reply_text(rtl("⚠️ اكتب 'نعم' أو 'لا' فقط:"))
        return CONFIRM_DELETE

    return ConversationHandler.END

async def execute_delete(method, param):
    """تنفيذ عملية الحذف"""
    if method == "all":
        cursor.execute("SELECT COUNT(*) FROM operations")
        ...
        month_str = f"{int(param):02d}"
        cursor.execute("SELECT COUNT(*) FROM operations WHERE strftime('%m', date) = ?", (month_str,))
        count = cursor.fetchone()[0]
        cursor.execute("DELETE FROM operations WHERE strftime('%m', date) = ?", (month_str,))
        conn.commit()
        return count
        ...
        cursor.execute("SELECT COUNT(*) FROM operations WHERE date = ?", (param,))
        count = cursor.fetchone()[0]
        cursor.execute("DELETE FROM operations WHERE date = ?", (param,))
        conn.commit()
        return count

    elif method == "hospital":
        cursor.execute("SELECT COUNT(*) FROM operations WHERE hospital LIKE ?", (f"%{param}%",))
        count = cursor.fetchone()[0]
        cursor.execute("DELETE FROM operations WHERE hospital LIKE ?", (f"%{param}%",))
        conn.commit()
        return count
...
# ========= الدوال الناقصة =========
async def generate_monthly_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        cursor.execute("""
        ...
        if not data:
            await update.message.reply_text(rtl("⚠️ لا توجد بيانات للإحصائيات الشهرية"))
            return
        ...
        await update.message.reply_text(rtl("❌ حدث خطأ أثناء إنشاء الإحصائيات الشهرية"))

async def generate_department_distribution(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        cursor.execute("""
        ...
        if not data:
            await update.message.reply_text(rtl("⚠️ لا توجد بيانات لتوزيع الأقسام"))
            return
        ...
        await update.message.reply_text(rtl("❌ حدث خطأ أثناء إنشاء توزيع الأقسام"))

async def generate_doctor_performance(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        cursor.execute("""
        ...
        if not data:
            await update.message.reply_text(rtl("⚠️ لا توجد بيانات لأداء الأطباء"))
            return
        ...
        await update.message.reply_text(rtl(performance_text), parse_mode='Markdown')

    except Exception as e:
        logger.error(f"Error in generate_doctor_performance: {e}")
        await update.message.reply_text(rtl("❌ حدث خطأ أثناء إنشاء تقرير أداء الأطباء"))

async def generate_department_comparison(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        cursor.execute("""
        ...
        if not data:
            await update.message.reply_text(rtl("⚠️ لا توجد بيانات للمقارنة بين الأقسام"))
            return
        ...
        plt.xticks(range(len(departments)), departments, rotation=45, ha='right')
        ...
        await update.message.reply_text(rtl("❌ حدث خطأ أثناء إنشاء مقارنة الأقسام"))
...
🗑️ **حذف البيانات**:
/delete - حذف العمليات (الكل، حسب الشهر، التاريخ، أو المستشفى)
...
def main():
    ...
    delete_conv = ConversationHandler(
        entry_points=[CommandHandler("delete", delete_operations)],
        states={
            CHOOSE_DELETE_METHOD: [MessageHandler(filters.TEXT & ~filters.COMMAND, choose_delete_method)],
            GET_DELETE_PARAM: [MessageHandler(filters.TEXT & ~filters.COMMAND, get_delete_param)],
    ...
    app.add_handler(CommandHandler("department_dist", generate_department_distribution))
    app.add_handler(CommandHandler("doctor_perf", generate_doctor_performance))
    app.add_handler(CommandHandler("compare_departments", generate_department_comparison))
